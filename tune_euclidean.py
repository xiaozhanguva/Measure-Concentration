...
if __name__ == "__main__":
	args = ld.argparser(dataset='mnist', epsilon=1.58, alpha=0.01, delta=0.25)
	# args = ld.argparser(dataset='cifar', epsilon=0.2453, alpha=0.05, delta=0)
	setproctitle.setproctitle('python')

	print('dataset: {dataset}\t\t' 'metric: {metric}\t\t'
		  'epsilon: {epsilon}\t\t' 'alpha: {alpha}\t\t'
		  'delta: {delta}\t\t'.format(dataset=args.dataset, metric=args.metric, 
			epsilon=args.epsilon, alpha=args.alpha, delta=args.delta))
	...
	#### coarse tuning over total number of balls 
	cluster_arr = np.arange(5, 21, 5)
	print('Searching grid for number of balls:', cluster_arr)
	print('')

	res_file = open(res_filepath+'/epsilon_'+str(args.epsilon)+'.txt', "w")
	for n_clusters in cluster_arr:
		print('==================== current number of balls available: '+str(n_clusters))
		...
		for t in range(n_clusters):
			...
			n_lower = math.ceil((n_point_tot-len(index_init)) / (n_clusters-t))
			n_upper = n_point_tot-len(index_init)
			print('number of points to cover: from '+str(n_lower)+' to '+str(n_upper))
			...
			for i in range(n_train):
				...
				## record the minimum expansion (ratio) and its index
		...
		#### save the results
		print(n_clusters, '{:.2%}'.format(risk_train), 
